...
# --------- 3. MODEL DEFINITION (CAE, bottleneck=8x8x8=512) ---------
...
with h5py.File('cae_feat_train_cifar10.h5', 'w') as hf:
    hf.create_dataset("features", data=latent_features_train_flat)
    hf.create_dataset("labels", data=y_train)
with h5py.File('cae_feat_val_cifar10.h5', 'w') as hf:
    hf.create_dataset("features", data=latent_features_val_flat)
    hf.create_dataset("labels", data=y_val)
with h5py.File('cae_feat_test_cifar10.h5', 'w') as hf:
    hf.create_dataset("features", data=latent_features_test_flat)
    hf.create_dataset("labels", data=y_test)
print("Latent features for train, validation, and test saved separately as cae_feat_train_cifar10.h5, cae_feat_val_cifar10.h5, cae_feat_test_cifar10.h5.")
...
decoded_imgs_flat = decoded_imgs_test.reshape(-1, 32*32*3)
...
for digit in range(10):
    ...
    for i, idx in enumerate(idxs):
        axes[0, digit].imshow(X_test[idx])
        axes[0, digit].axis('off')
        if i == 0:
            axes[0, digit].set_title(f"Class {digit}", fontsize=12)
        axes[1, digit].imshow(decoded_imgs_test[idx])
