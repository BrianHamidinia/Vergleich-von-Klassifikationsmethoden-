...
def unet_autoencoder(input_shape=(32,32,3)):
    ...
    p3 = Dropout(0.2)(p3)
    # Bottleneck
    b = Conv2D(8, (3,3), activation='relu', padding='same')(p3)
    b = BatchNormalization()(b)
    bottleneck = Dropout(0.2)(b)
    # Decoder
    u3 = UpSampling2D((2,2))(b)
    u3 = Concatenate()([u3, c3])
    d3 = Conv2D(256, (3,3), activation='relu', padding='same', kernel_regularizer=l2(1e-4))(u3)
    ...
    outputs = Conv2D(3, (3,3), activation='sigmoid', padding='same')(d1)
...
def plot_3row_images(X_org, X_noisy, X_denoised, y_labels, n_classes=10, n_per_class=1):
    ...
    for digit in range(n_classes):
        ...
        if len(idxs) == 0:
            continue
        idx = idxs[0]
        axes[0, digit].imshow(X_org[idx])
        axes[0, digit].axis('off')
        axes[0, digit].set_title(f"Class {digit}", fontsize=10)
        axes[1, digit].imshow(X_noisy[idx])
        axes[1, digit].axis('off')
        axes[2, digit].imshow(X_denoised[idx])
...
os.makedirs("features_cae_cifar10", exist_ok=True)
with h5py.File('features_cae_cifar10/cae_feat_train.h5', 'w') as hf:
    hf.create_dataset("features", data=latent_train_flat)
    hf.create_dataset("labels", data=y_train)
with h5py.File('features_cae_cifar10/cae_feat_val.h5', 'w') as hf:
    hf.create_dataset("features", data=latent_val_flat)
    hf.create_dataset("labels", data=y_val)
with h5py.File('features_cae_cifar10/cae_feat_test.h5', 'w') as hf:
